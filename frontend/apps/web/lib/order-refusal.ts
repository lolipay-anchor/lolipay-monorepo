export const IDENTITY_REQUIRED =
  'Verify your identity before your first trade. Open Profile to start it.'
...
const PLAIN_WORDS: Array<[RegExp, string]> = [
  ...
  [/no eligible LP available/i, 'No provider can take this order right now. This can last hours — try again later.'],
...
export function keepIdentity(current: string | null): string | null {
  return current === IDENTITY_REQUIRED ? current : null
}
