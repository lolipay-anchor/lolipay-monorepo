...
#[test]
fn a_resolver_alone_can_reach_a_bond_the_same_day_it_settles_and_adr_0022_accepts_this() {
    let s = slash_setup();
    s.usdc_admin.mint(&s.lp, &2_000_000_000i128);
    s.staking.stake(&s.lp, &2_000_000_000i128);

    let trade_id = s.make_trade(60, false);
    s.escrow.mark_fiat_paid(&trade_id, &s.lp);
    let now = s.env.ledger().timestamp();
    s.env.ledger().with_mut(|li| li.timestamp = now + 1);
    s.escrow.confirm_and_release(&trade_id);

    let settled = s.env.ledger().timestamp();
    s.env.ledger().with_mut(|li| li.timestamp = settled + 1);
    s.escrow.raise_dispute(&trade_id, &s.resolver);
    s.escrow.resolve(&trade_id, &ResolveOutcome::Refund, &s.resolver);
...
#[test]
#[should_panic(expected = "Error(Contract, #11)")]
fn a_cooldown_equal_to_the_remedy_window_is_refused_because_a_tie_is_decided_by_order() {
    let env = Env::default();
    env.mock_all_auths();
    let admin = Address::generate(&env);
    let usdc = Address::generate(&env);
...
#[test]
fn a_cooldown_one_second_past_the_remedy_window_is_accepted() {
    let env = Env::default();
    env.mock_all_auths();
    let admin = Address::generate(&env);
    let usdc = Address::generate(&env);
