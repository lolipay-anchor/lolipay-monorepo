...
import { personIdForWallet } from '../lp/lp.service';
    ...
    const committed = await this.prisma.$transaction(async (tx) => {
      ...
      if (minOrderPatch !== undefined || patch.dailyLimitByTier !== undefined) {
        ...
        if (belowFloor.length > 0) {
          throw new Error(dailyLimitBelowMinOrderMessage(belowFloor, nextMinOrder));
        }
      }
      ...
      const updated = await tx.config
        ...
        .catch((e: unknown) => {
          if (e instanceof Prisma.PrismaClientKnownRequestError && e.code === 'P2025') {
            throw new ConflictException(CONFIG_WRITE_CONFLICT_SENTENCE);
          }
          throw e;
        });
