...
describe('resolveRole', () => {
  ...
  it('returns admin for an allowlisted address (never even queries the DB)', async () => {
    ...
    await expect(resolveRole(ADMIN, prisma, [ADMIN])).resolves.toBe('admin');
  ...
  it('returns lp for an APPROVED Lp row', async () => {
    ...
    await expect(resolveRole(LP_ADDR, prisma, [ADMIN])).resolves.toBe('lp');
  });
  ...
  it('returns user for a PENDING/suspended Lp row (not yet/no-longer approved)', async () => {
    ...
    await expect(resolveRole(LP_ADDR, prisma, [ADMIN])).resolves.toBe('user');
  });

  it('returns user for an address with no Lp row at all', async () => {
    const prisma = makePrisma(null);
    await expect(resolveRole(USER_ADDR, prisma, [ADMIN])).resolves.toBe('user');
  });
