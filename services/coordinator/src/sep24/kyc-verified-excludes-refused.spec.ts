...
describe('kyc_verified on a SEP-24 transaction record', () => {
  it('is true only for a person whose acceptance the gate honours and who carries no refusal under any subject', async () => {
    ...
    const verified = await (svc as any).screenedPeople(['p-clean', 'p-refused', 'p-stub', 'p-unknown']);
    expect([...verified].sort()).toEqual(['p-clean']);
  });

  it('still refuses a stub acceptance and a refused person when AML is optional', async () => {
    const { svc } = service(false);
    const verified = await (svc as any).screenedPeople(['p-clean', 'p-refused', 'p-stub']);
    expect([...verified].sort()).toEqual(['p-clean']);
  });
