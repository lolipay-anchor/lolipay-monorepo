...
describe('the more-info page of a deposit', () => {
  ...
  it('shows no bank details once the deposit has left FUNDED, and never for a withdrawal', async () => {
    const released = await service({ ...baseOrder, status: 'RELEASED', settlementTxHash: HASH, settledAt: new Date(), lpPaymentDetails: 'BCA 1234567890', payDeadline: 4_000_000_000n }).moreInfo('tx-1');
    expect(released).not.toContain('BCA 1234567890');
    expect(released).not.toContain('Send <strong>');
    const withdrawal = service({ ...baseOrder, status: 'FUNDED', settlementTxHash: null, settledAt: null, lpPaymentDetails: 'BCA 1234567890', flow: 'WITHDRAW', payDeadline: 4_000_000_000n });
    (withdrawal as any).prisma.sep24Transaction.findUnique = jest.fn(async () => ({
      ...
      order: { personId: 'person-1', ...baseOrder, status: 'FUNDED', settlementTxHash: null, settledAt: null, lpPaymentDetails: 'BCA 1234567890', flow: 'WITHDRAW', payDeadline: 4_000_000_000n },
    }));
    const html = await withdrawal.moreInfo('tx-1');
    expect(html).not.toContain('BCA 1234567890');
  });

  it('asks for no money when the order carries no pay deadline at all, because the JSON already says the time has passed', async () => {
    const html = await service({ ...baseOrder, status: 'FUNDED', settlementTxHash: null, settledAt: null, lpPaymentDetails: 'BCA 1234567890', ref: 'LP-42', payDeadline: 0n }).moreInfo('tx-1');
    expect(html).not.toContain('BCA 1234567890');
    expect(html).not.toContain('Send <strong>');
    expect(html).toContain('The time to send the rupiah has passed');
  });
