...
const OPTED_IN: Array<[string, string, number]> = [
  ['matching/matching.service.ts', "new ServiceUnavailableException('no eligible LP available')", 1],
  ['order/order.service.ts', "new ServiceUnavailableException('no eligible LP available')", 1],
  ['kyc/sep12.service.ts', "new ForbiddenException('this identity was refused and cannot be resubmitted here')", 2],
];
...
describe('every refusal this commit opted in still carries its marker at the throw', () => {
  it.each(OPTED_IN)('%s wraps every %s, and there are %i of them', (relative, construction, expected) => {
