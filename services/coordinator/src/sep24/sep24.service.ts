  ...
  async submitAmount(
    ...
    if (linked.count !== 1) {
      ...
      if (undone.count === 1) {
        ...
        this.log.error(
          `a second order ${orderId} was opened for SEP-24 transaction ${id} and could NOT be cancelled (${undone.count} rows matched); it holds provider capacity until it expires`,
        );
      }
    }
  }
